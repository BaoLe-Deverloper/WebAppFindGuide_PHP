...
(function($) {
        ...
        $.fn.timeLine = function(prop)
        {
            ...
            $(document).ready(function()
            {
                ...
                {
                    'use strict';
                    title.eq(iteration).animate({bottom: '10px' ,opacity: 1}, 500);
                }
                function playVideo()
                {
                    video.after('<div class="controll"><i style="font-size: 50px" class="fa fa-play-circle" aria-hidden="true"></i></div>');
                ...
                function init()
                {
                    ...
                    title.click(function(e)
                    {
                        ...
                        {
                            $(e.target).closest('.caption').find('.textContent').slideDown(200).addClass('in');
                            $(e.target).closest('.caption').find('.title i').css({transform: 'rotate(90deg)', transition: 'transform 0.4s ease-out'});
                        }else
                        {
                            $(e.target).closest('.caption').find('.textContent').slideUp(200).removeClass('in');
                ...
                {
                    'use strict';
                    itemDuration = (settings.itemAnimateDuration - (settings.itemAnimateDuration * 0.75)) * 700;
